...
function validateForm() {
    ...
    if (firstname === '') {
        setError(firstnameInput, "Un pré est requis.");
        isValid = false;
    } else if (firstname.length > 30) {
        setError(firstnameInput, "Le pré ne peut pas dépasser 30 caractères.");
        isValid = false;
    } else {
    ...
    if ( === '') {
        setError(Input, "Un  est requis.");
        isValid = false;
    } else if (.length > 50) {
        setError(Input, "Le  ne peut pas dépasser 50 caractères.");
        isValid = false;
    } else {
        setSuccess(Input);
    }
