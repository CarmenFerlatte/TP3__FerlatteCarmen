...
console.log('Salut World');
    ...
    function setError(input, message) {
        ...
        const errorSpan = document.getElementById(input.prenom + '-Error', input.nom + '-Error', input.email + '-Error', input._subject + '-Error', input.message + '-Error');
        if (errorSpan) {
            errorSpan.textContent = message;
        } else {
            console.error("Erreur le span n'a pas été trouvé", input.prenom); // console.error() permet d'afficher une erreur dans la console
        }
    }

    function setSuccess(input) {
        input.style.border = '3px solid green';
        const errorSpan = document.getElementById(input.prenom + '-Error', );
        if (errorSpan) {
            errorSpan.textContent = '';
        } else {
            console.error("Erreur le span n'a pas été trouvé", input.prenom);
        }
}
