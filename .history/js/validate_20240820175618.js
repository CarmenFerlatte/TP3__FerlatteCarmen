...
function validateForm() {
    ...
    if (firstname === '') {
        setError(firstnameInput, "Un prélastname est requis.");
        isValid = false;
    } else if (firstname.length > 30) {
        setError(firstnameInput, "Le prélastname ne peut pas dépasser 30 caractères.");
        isValid = false;
    } else {
        setSuccess(firstnameInput);
    }

    const lastnameInput = document.forms['form']['lastname'];
    const lastname = lastnameInput.value.trim();
    if (lastname === '') {
        setError(lastnameInput, "Un lastname est requis.");
        isValid = false;
    } else if (lastname.length > 50) {
        setError(lastnameInput, "Le lastname ne peut pas dépasser 50 caractères.");
        isValid = false;
    } else {
