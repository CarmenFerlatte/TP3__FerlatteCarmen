...
console.log('Salut le monde');
    ...
    const selection = document.getElementById('plan');
    const entretienSelect = document.getElementById('entretien');

    // Ceci vérifie si l'une des options est sélectionnée    
    if (planSelect.value === 'selectionner' && entretienSelect.value === 'selectionner') {
        setError(planSelect, "Veuillez sélectionner un plan et/ou un forfait entretien.");
        setError(entretienSelect, "Veuillez sélectionner un plan et/ou un forfait entretien.");
        isValid = false;
} else {
    
    // Si au moins une option est sélectionnée, considérée le formulaire comme valide
    if (planSelect.value !== 'selectionner') {
        setSuccess(planSelect);
    } else {
        setError(planSelect, "Veuillez sélectionner un plan.");
    }
    
    if (entretienSelect.value !== 'selectionner') {
        setSuccess(entretienSelect);
    } else {
        setError(entretienSelect, "Veuillez sélectionner un entretien.");
    }
}
    
    console.log("Le formulaire est valide : " + isValid);
    return isValid;
    }
